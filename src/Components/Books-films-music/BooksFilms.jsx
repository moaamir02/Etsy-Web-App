...
import BooksFilmsCard from './BooksFilmsCard'; // Assuming this is the correct import
import { BooksFilmsData } from '../../Constant';
...
const BooksFilms = () => {
    ...
    const firstRowImages = BooksFilmsData.slice(0, imagesPerPage);
    // Slicing the data to get the images for the second row if showMore is true
    const secondRowImages = showMore ? BooksFilmsData.slice(imagesPerPage, imagesPerPage * 2) : [];
    // Slicing the data to get the remaining images if showMore is true
    const remainingImages = showMore ? BooksFilmsData.slice(imagesPerPage * 2) : [];
    ...
    return (
      <div>
        ...
        <div className="heading-para mt-8">
          <h1 className="heading text-center text-4xl font-thin text-gray-800">Books, Films & Music</h1> {/* Updated the heading */}
          <p className="text-center mt-2 text-gray-700">
          Celebrate creativity with planners, journals, and blank notebooks, or music, films, and books

 {/* This description might also need updating */}
          </p>
        </div>
