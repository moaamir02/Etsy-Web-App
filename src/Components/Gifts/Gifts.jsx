...
import { GiftsData } from '../../Constant'; // Importing the data for accessories images
...
const Gifts = () => {
  ...
  const firstRowImages = GiftsData.slice(0, imagesPerPage);
  // Slicing the data to get the images for the second row if showMore is true
  const secondRowImages = showMore ? GiftsData.slice(imagesPerPage, imagesPerPage * 2) : [];
  // Slicing the data to get the remaining images if showMore is true
  const remainingImages = showMore ? GiftsData.slice(imagesPerPage * 2) : [];
  ...
  return (
    <div>
      ...
      <div className="heading-para mt-8">
        <h1 className="heading text-center text-4xl font-thin text-gray-800">Gifts</h1>
